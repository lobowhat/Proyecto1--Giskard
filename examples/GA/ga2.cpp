...
int ga2()
{
    ...
    //just loop std::endlessly until user gets bored :0)
    while (true) {
        ...
        while (!bFound) {
            ...
            for (int i = 0; i < POP_SIZE; i++) {
                if (Population[i].fitness == 999.0f) {
                    std::cout << "\nSolution found in " << GenerationsRequiredToFindASolution << " generations!" << std::endl << std::endl;;
            ...
            if (GenerationsRequiredToFindASolution > MAX_ALLOWABLE_GENERATIONS) {
                std::cout << "No solutions found this run!";
